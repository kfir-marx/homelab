...
Family and friends
        |
        | HTTPS: jellyfin / seerr / immich / tapy public hostnames
        v
...
| Tapy | `tapy.547600.xyz` | `tapy-frontend.tapy.svc:3000` | No tunnel-layer authentication; `/v1/*` is proxied to `tapy-backend.tapy.svc:8080` for its own session and OAuth controls |
...
| Media administration | `https://{seerr,sonarr,radarr,whisparr,prowlarr,bazarr,qbittorrent,maintainerr}.home.547600.xyz` | Cilium Gateway API with cert-manager TLS |
| AdGuard Home DNS | `192.168.1.221:53` TCP/UDP | Cilium LoadBalancer |
| qBittorrent peers | `192.168.1.222:51413` TCP/UDP | Cilium LoadBalancer; router forwards WAN TCP/UDP 51413 |
