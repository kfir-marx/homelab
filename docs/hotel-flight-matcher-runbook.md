...
Create `homelab-assistant/hotel-flight-matcher-secrets` with:
...
`scripts/secrets.sh capture-k8s homelab-assistant/hotel-flight-matcher-secrets`.

Before sync, use the Ansible workstation play to create
`/mnt/storage2-bulk/hotel-flight-matcher/postgres`; do not create or alter the
...
RabbitMQ, internal-llm, external-ai, and finally hotel-flight-matcher. Do not
