...
export default function AppShell() {
  ...
  useEffect(() => {
    function read() { const token = new URLSearchParams(location.hash.slice(1)).get("invite") || sessionStorage.getItem("tapy-invitation") || ""; if (token) sessionStorage.setItem("tapy-invitation", token); setInvitation(token); }
    read(); addEventListener("hashchange", read); return () => removeEventListener("hashchange", read);
  }, []);
  if (invitation) return <InvitationScreen token={invitation} done={() => setInvitation("")} />;
  if (loading) return <main className="grid min-h-screen place-items-center text-sm text-slate-500">Loading Tapy…</main>;
  if (!user) return <AuthScreen />;
  if (user.invitation_required) return <main className="grid min-h-screen place-items-center p-8"><section><h1 className="text-2xl font-semibold">Invitation required</h1><p className="my-4">Signed in as {user.email}. Ask an organization admin for an invitation link.</p><button className="underline" onClick={() => void logout()}>Sign out</button></section></main>;
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">{view === "personal" ? <AgentView /> : view === "organization" ? <AgencyView /> : view === "profile" ? <UserInfoPage /> : <><UserSettingsPage />{user.active_organization_role === "admin" && <Team key={user.active_organization_id} />}</>}</main>
